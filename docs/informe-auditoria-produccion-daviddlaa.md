...
> **Actualización (17/08/2026):** como resultado de esta auditoría, la **Fase 1** del plan de instrumentación ya está implementada: el sistema ahora registra `duracion_seg`, `llamada_inicio`, `llamada_fin`, `resultado` (estructurado) y `metodo_duracion` en cada gestión, con un **popup de llamada con contador** que se abre desde el botón 📞 de cada tarjeta (móvil). Ver `docs/plan-metricas-llamadas-semaforo.md`. Las secciones 2 y 9 de este informe describen el estado **antes** de esa instrumentación.
