...
| Widget "🕘 Últimas gestiones" en el Dashboard (móvil + escritorio): timeline de las últimas 5 gestiones (líder → su equipo, resto → propias); en escritorio el grid de 2 widgets se convierte en pasarela deslizable de 3 slides con flechas y dots | Agosto 2026 | `docs/feature-widget-ultimas-gestiones-dashboard.md` |
| Modal "Agregar a Campaña" en Solicitudes (móvil + escritorio): botón confirmar arriba junto al título, toast de éxito con nombre de campaña, limpieza de selección y refresco en vivo del badge (móvil ya no redirige a la campaña) | Agosto 2026 | `docs/feature-ux-agregar-campana-solicitudes.md` |
| Campaña Completada sale del semáforo en gestion-lote (móvil + escritorio): oculta barra/tarjetas del semáforo, nota "semáforo desactivado", filtros no cuentan; al re-activar se restaura | Agosto 2026 | `docs/feature-semaforo-campana-completada.md` |
| Fix importación Excel: nunca modifica ni reasigna solicitudes de otros usuarios (las omite y reporta con ⚠️ en el informe de importación) | Agosto 2026 | `docs/fix-importacion-proteccion-datos-usuarios.md` |
| Excel de datos demo para el video: 28 solicitudes ficticias (IDs auto-generados, estados/segmentos/vendedores del catálogo) en `docs/demo/archivox-datos-demo.xlsx` | Agosto 2026 | `docs/feature-excel-demo-video.md` |
| Vista móvil de Solicitudes (UX v2): leyenda de filtros + fila única, fechas colapsables y disponibles para todos los usuarios, KPIs 20% más compactos, buscador con botón ✕, selects y "Seleccionar todo" compactos (32 px). Segunda iteración: buscador + "Seleccionar todo" integrados dentro del panel bajo el toggle de filtros, y fix crítico del menú ⋮ que no se desplegaba (la card retenía un transform que la volvía containing block del menú fixed; se eliminó del keyframe `fadeInUp` y del `:active`) | Agosto 2026 | `docs/feature-filtros-buscador-movil-solicitudes.md` |
| Prioridad por tiempo sin seguimiento en campañas (móvil + escritorio): al filtrar por Encaminadas/Seguimiento/En espera, las solicitudes se ordenan por las de mayor tiempo sin una gestión (sin gestiones primero, luego por fecha de última gestión más antigua), con toast informativo y badge ⏱️ por tarjeta | Agosto 2026 | `docs/feature-prioridad-tiempo-sin-seguimiento.md` |
| Límite del texto de seguimiento en tarjetas de campaña: 2 líneas en móvil (`.sol-obs`, tap para expandir) y 4 líneas en escritorio (`.sol-ultima-gestion-obs`); tarjetas con altura uniforme sin perder el contexto | Agosto 2026 | `docs/feature-limite-texto-seguimiento-tarjetas.md` |
| Búsqueda global de solicitudes en el landing de Campañas (móvil + escritorio): barra que busca la solicitud en todas las campañas vía `GET /api/excel/solicitudes/buscar`; al hacer clic navega a la campaña con deep link `?id=&card=` (salta y resalta la tarjeta). El escritorio ahora soporta el parámetro `?card=` | Agosto 2026 | `docs/feature-buscador-global-campanas.md` |
| Corrección del flujo de guardado de seguimientos en campañas: contador `gestionadas` recalculado por solicitud (no por fila), control de acceso en `POST /api/excel/gestiones`, eliminación del endpoint muerto `createGestion`, criterio de última gestión unificado por `id DESC`, destacar aislado (evita duplicados), update local de la tarjeta sin perder filtro/scroll, foco inicial en el textarea y toast por tipo. Lógica compartida en `public/js/gestion-campana.js` | Agosto 2026 | `docs/informe-correccion-flujo-seguimiento-campanas.md` |
| Rediseño del Panel del Líder móvil (`/m/equipo`): experiencia tipo app con 3 pestañas (Agentes / Campañas / Actividad), KPI strip compacto, buscador y orden de agentes, detalle de agente en pantalla completa (campañas y gestiones del agente), campañas clicables, timeline de actividad agrupado por día con paginación y CSS dedicado `public/movil/css/equipo.css` | Agosto 2026 | `docs/feature-rediseno-equipo-movil.md` |
| Cédula visible en la tarjeta móvil de Solicitudes: 🆔 + número **debajo del nombre**, y nombre con letra más pequeña + ellipsis para que **nunca** se parta en 2 líneas (antes la cédula solo estaba en el modal de detalle) | Agosto 2026 | `docs/feature-cedula-tarjeta-movil-solicitudes.md` |
