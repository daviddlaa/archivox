...
async function up() {
  try {
    console.log('➕ Agregando columna vendedor a gestiones...');
    await db.query(`ALTER TABLE gestiones ADD COLUMN vendedor TEXT;`);
    console.log('✅ Columna vendedor agregada correctamente.');
  } catch (error) {
    if (error.message && (error.message.includes('duplicate column') || error.message.includes('already exists'))) {
      console.log('ℹ️ La columna vendedor ya existe. Saltando...');
    } else {
